use crate::{
    ColorMode, FontSystem, GlyphDetails, GlyphToRender, GpuCacheStatus,
    PrepareError, RenderError, SwashCache, SwashContent, TextArea, TextAtlas,
    Viewport,
};
...
use wgpu::{
    ...
    CommandEncoder, DepthStencilState, Device, Extent3d, MultisampleState,
    Origin3d, Queue, RenderPass, RenderPipeline, TexelCopyBufferLayout,
...
impl TextRenderer {
    ...
    pub fn new(
        ...
        let pipeline =
            atlas.get_or_create_pipeline(device, multisample, depth_stencil);
    ...
    pub fn prepare_with_depth<'a>(
        ...
        for text_area in text_areas {
            ...
            let bounds_max_x =
                text_area.bounds.right.min(resolution.width as i32);
            let bounds_max_y =
                text_area.bounds.bottom.min(resolution.height as i32);

            let is_run_visible = |run: &cosmic_text::LayoutRun| {
                let start_y_physical =
                    (text_area.top + (run.line_top * text_area.scale)) as i32;
                let end_y_physical = start_y_physical
            ...
            for run in layout_runs {
                for glyph in run.glyphs.iter() {
                        ...
                        atlas.mask_atlas.glyph_cache.get(&cache_key)
                    {
                        ...
                        atlas.color_atlas.glyph_cache.get(&cache_key)
                    {
                        ...
                        let (gpu_cache, atlas_id, inner) = if should_rasterize {
                            let mut inner =
                                atlas.inner_for_content_mut(content_type);
                            ...
                            let allocation = loop {
                                match inner.try_allocate(width, height) {
                                    ...
                                    None => {
                                        ...
                                        inner = atlas.inner_for_content_mut(
                                            content_type,
                                        );
                                    }
                                }
                        ...
                        inner.glyph_cache.get_or_insert(cache_key, || {
                            GlyphDetails {
                    ...
                    let (mut atlas_x, mut atlas_y, content_type) =
                        match details.gpu_cache {
        ...
        if self.vertex_buffer_size >= vertices_raw.len() as u64 {
            self.staging_belt
                .write_buffer(
                    ...
                    &self.vertex_buffer,
                    0,
                    NonZeroU64::new(vertices_raw.len() as u64)
                        .expect("Non-empty vertices"),
                )
...
fn create_oversized_buffer(
    ...
    buffer.slice(..).get_mapped_range_mut().slice(..contents.len())
